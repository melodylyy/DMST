...
class SimilarityFusion:
    ...
    def calculate_fusion(self):
        ...
        drug_gip = self.GIP_sm(self.A.cpu().numpy())
        miRNA_gip = self.GIP_m(self.A.cpu().numpy())


        miRNA_bio = self.miRNA.cpu().numpy()
        drug_bio = self.drug.cpu().numpy()


        return (
            th.from_numpy(miRNA_bio).to(self.device),
            th.from_numpy(miRNA_gip).to(self.device),
            th.from_numpy(drug_bio).to(self.device),
            th.from_numpy(drug_gip).to(self.device)
        )
